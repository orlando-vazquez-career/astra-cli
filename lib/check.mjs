...
export function scanFile(file, text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (/(mnemonic|seed[ _-]?phrase|frase semilla)/i.test(line)) {
      // Una frase semilla real son 12 a 24 palabras CONSECUTIVAS de 3 a 8 letras (BIP-39),
      // separadas por un espacio; la prosa que menciona "mnemonic" no cumple eso.
      const phrase = /(?:\b[a-z]{3,8}\b ){11,23}\b[a-z]{3,8}\b/.test(line);
      if (phrase && !isExample) {
